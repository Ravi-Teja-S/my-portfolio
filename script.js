...
//   /* Show native crosshair and hide the custom DOM cursor to prevent lag overlap */
//   .armed, .armed * { cursor: crosshair !important; }
//   .armed #custom-cursor, .armed #custom-cursor-trail { display: none !important; }
...
//   mouseX = e.pageX; // Accounts for scrolling
//   mouseY = e.pageY; // Accounts for scrolling
// });
...
//   gun.style.top = (window.scrollY + 100 + Math.random() * (window.innerHeight - 200)) + 'px';
...
//     if (state === 'normal') {
//       x += vx; y += vy;
//      // Let them bounce off the full height of the website!
//       if(x <= 0) { x = 0; vx = Math.abs(vx); }
//       else if(x >= window.innerWidth - 60) { x = window.innerWidth - 60; vx = -Math.abs(vx); }
//       if(y <= 80) { y = 80; vy = Math.abs(vy); }
//       else if(y >= document.body.scrollHeight - 60) { y = document.body.scrollHeight - 60; vy = -Math.abs(vy); }
//     } 
//     else if (state === 'chase') {
...
//            // NEW: Auto-focus the invisible mobile input so the keyboard pops up!
//            const mobileInput = document.getElementById('reboot-mobile-input');
//            if(mobileInput) {
//              mobileInput.value = "";
...
// // 1. Create an invisible input so mobile devices can open their software keyboard
// const rebootMobileInput = document.createElement('input');
// rebootMobileInput.id = 'reboot-mobile-input';
// rebootMobileInput.style.position = 'absolute';
// rebootMobileInput.style.opacity = '0';
// rebootMobileInput.style.width = '1px';
// rebootMobileInput.style.height = '1px';
// rebootMobileInput.style.border = 'none';
// rebootMobileInput.setAttribute('autocomplete', 'off');
// rebootMobileInput.setAttribute('spellcheck', 'false');
// document.body.appendChild(rebootMobileInput);

// // 2. Sync mobile typing to the fake terminal text
// rebootMobileInput.addEventListener('input', (e) => {
//   if (!isSystemOffline) return;
//   rebootBuffer = e.target.value.substring(0, 25);
//   document.getElementById('reboot-input').textContent = rebootBuffer;
//   document.getElementById('reboot-sys-msg').textContent = "SYSTEM OFFLINE. AWAITING COMMAND.";
//   document.getElementById('reboot-sys-msg').style.color = "var(--text-dimmer)";
// });

// // 3. Handle Enter key from the mobile keyboard
// rebootMobileInput.addEventListener('keydown', (e) => {
//   if (!isSystemOffline) return;
//   if (e.key === 'Enter') {
...
//        rebootMobileInput.blur(); // Hide keyboard
//        startMatrixReboot();
//     } else {
...
//        rebootBuffer = "";
//        rebootMobileInput.value = "";
//        document.getElementById('reboot-input').textContent = "";
//     }
//   }
// });

// // Keep input focused if they tap the screen while offline (brings keyboard back if dismissed)
// document.addEventListener('click', () => {
//   if (isSystemOffline) {
//     rebootMobileInput.focus({ preventScroll: true });
//   }
// });

// // 4. Desktop Keydown Listener (Kept intact for physical keyboards)
// document.addEventListener('keydown', (e) => {
//   if (!isSystemOffline) return;
//   if (document.activeElement === rebootMobileInput) return; // Let mobile input handle it if focused
...
/* ---------------- THEME ---------------- */
const THEMES = [
  { name:'sky', accent:'#7dd3fc' }, { name:'mint', accent:'#6ee7b7' },
  { name:'rose', accent:'#fda4af' }, { name:'amber', accent:'#fcd34d' },
  { name:'violet', accent:'#c4b5fd' }
];
function applyTheme(t) {
  document.documentElement.style.setProperty('--accent', t.accent);
...
weaponStyle.innerHTML = `
  .armed, .armed * { cursor: crosshair !important; }
  .armed #custom-cursor, .armed #custom-cursor-trail { display: none !important; }
  .ascii-gun { animation: float-gun 2s ease-in-out infinite; font-weight: bold; text-shadow: 0 0 8px var(--accent); }
  @keyframes float-gun { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-10px); } }
  .armed .pet { transition: outline 0.1s, background 0.1s, box-shadow 0.1s; }
  .armed .pet:hover { outline: 2px dashed #ff5f56; outline-offset: 6px; background-color: rgba(255, 95, 86, 0.15); border-radius: 2px; box-shadow: inset 0 0 10px rgba(255, 95, 86, 0.3), 0 0 15px rgba(255, 95, 86, 0.5); }
  .armed .pet:hover::after { content: '[ LOCKED ]'; position: absolute; bottom: -22px; left: 50%; transform: translateX(-50%); color: #ff5f56; font-size: 10px; font-weight: bold; letter-spacing: 2px; white-space: nowrap; animation: boot-blink 0.5s step-end infinite; }
  @keyframes screen-recoil { 0% { transform: translate(0, 0) rotate(0deg); filter: contrast(100%) brightness(1); } 20% { transform: translate(-12px, 12px) rotate(-1.5deg); filter: contrast(140%) brightness(1.2); } 40% { transform: translate(12px, -12px) rotate(1.5deg); filter: contrast(100%) brightness(1); } 60% { transform: translate(-6px, 6px) rotate(-0.5deg); } 80% { transform: translate(6px, -6px) rotate(0.5deg); } 100% { transform: translate(0, 0) rotate(0deg); filter: contrast(100%) brightness(1); } }
  body.recoil .wrap, body.recoil .topbar, body.recoil .pet-layer { animation: screen-recoil 0.25s cubic-bezier(0.36, 0.07, 0.19, 0.97) both; }
`;
document.head.appendChild(weaponStyle);
...
function isEdge(glyph, row, col) {
  ...
  for(const [dr,dc] of dirs){
    const r = row+dr, c = col+dc;
    if(r<0 || r>=glyph.length || c<0 || c>=glyph[0].length) return true;
    if(glyph[r][c] !== '1') return true;
  }
  return false;
}
function pickChar(variant, edge, row, col, letterIdx) {
  switch(variant) {
    case 'shade': return (row+col)%2===0 ? '▓' : '▒';
    case 'sparse': return (row+col)%2===0 ? '#' : ' ';
    case 'mixed': return ['#','@','%','$'][(row+col+letterIdx)%4];
    default: return '█';
  }
}
const VARIANTS = ['solid', 'shade', 'sparse', 'mixed'];
    ...
    for(let row=0; row<glyph.length; row++){
      for(let sy=0; sy<2; sy++){
        ...
        for(let col=0; col<glyph[0].length; col++){
          const bit = glyph[row][col] === '1';
          line += (bit ? pickChar(variant, isEdge(glyph,row,col), row, col, li) : ' ').repeat(2);
        }
        rows[row*2+sy] += line + '  ';
      }
    }
...
function renderHero() {
  document.getElementById('hero-name').innerHTML = '<pre>' + buildHeroAscii('RAVI TEJA S', VARIANTS[variantIndex]) + '</pre>';
}
renderHero();
document.getElementById('hero-name').addEventListener('click', () => {
  applyTheme(randomTheme());
...
function onScroll() {
  ...
  let current = document.querySelectorAll('section')[0];
  document.querySelectorAll('section').forEach(s => {
    if(s.getBoundingClientRect().top < 140) current = s;
  });
  document.getElementById('crumbPath').textContent = '› ' + current.id;
}
window.addEventListener('scroll', onScroll, { passive:true });
onScroll();
...
function scrambleTextNode(el) {
  ...
  const walker = document.createTreeWalker(el, NodeFilter.SHOW_TEXT, null, false);
  let n;
  while(n = walker.nextNode()) if(n.textContent.trim().length > 0) textNodes.push({ node: n, original: n.textContent });
  let intId = setInterval(() => {
    textNodes.forEach(t => { t.node.textContent = t.original.split('').map(c => c.match(/[a-zA-Z0-9]/) ? chars[Math.floor(Math.random()*chars.length)] : c).join(''); });
  }, 60);
  return { stop: () => { clearInterval(intId); textNodes.forEach(t => t.node.textContent = t.original); } };
}
...
const layer = document.getElementById('petLayer');
const activePets = []; 
let isSystemOffline = false, rebootBuffer = "", eatenItems = [], triggerRestore = null; 
let isArmed = false, ammoCount = 0;

function spawnGun() {
  if (document.querySelectorAll('.ascii-gun').length > 1) return; 
  const gun = document.createElement('div');
  gun.className = 'ascii-gun pet'; 
  gun.style.color = '#7dd3fc';
  gun.innerHTML = `<span class="tip">EQUIP</span>⌐╦╦═─`;
  ...
  gun.addEventListener('pointerdown', (e) => {
      ...
      gun.remove();
      isArmed = true; ammoCount = 6;
      document.body.classList.add('armed');
      let ui = document.getElementById('ammo-counter');
      if(!ui) {
         ui = document.createElement('div'); ui.id = 'ammo-counter';
         ui.style.cssText = "position:fixed; bottom:40px; left:20px; color:#ff5f56; font-family:'JetBrains Mono',monospace; font-weight:bold; font-size:12px; z-index:1000; pointer-events:none;";
         document.body.appendChild(ui);
      }
      ui.innerHTML = `[ ARMAMENT: ⌐╦╦═─ ${ammoCount}/6 ]<br><span style="color:var(--text-dimmer); font-size:10px;">CLICK ANYWHERE TO SHOOT</span>`;
  });
...
window.addEventListener('pointerdown', (e) => {
  if (isArmed) {
      if (e.target.closest('.ascii-gun')) return; 
      document.body.classList.remove('recoil'); void document.body.offsetWidth; document.body.classList.add('recoil');
      setTimeout(() => document.body.classList.remove('recoil'), 250);
      const bang = document.createElement('div');
      bang.innerHTML = '<span style="color:#ffbd2e; font-size:18px;">💥</span>';
      bang.style.cssText = `position:fixed; left:${e.clientX}px; top:${e.clientY}px; pointer-events:none; z-index:9999; transform:translate(-50%,-50%);`;
      document.body.appendChild(bang);
      setTimeout(() => bang.remove(), 200);
      ammoCount--;
      const ui = document.getElementById('ammo-counter');
      if (ammoCount <= 0) { isArmed = false; document.body.classList.remove('armed'); if(ui) ui.remove(); } 
      else if(ui) ui.innerHTML = `[ ARMAMENT: ⌐╦╦═─ ${ammoCount}/6 ]<br><span style="color:var(--text-dimmer); font-size:10px;">CLICK TO FIRE</span>`;
  }
}, true);
...
function spawnPet(x, y, species){
  ...
  let state = 'normal', isRemoved = false; 
  
  const petData = {
    get x() { return x; }, get y() { return y; }, get state() { return state; },
    remove: () => { isRemoved = true; el.remove(); },
    explode: () => {
      if (state === 'blasted') return;
      state = 'blasted'; vx = 0; vy = 0; el.style.transition = 'none'; 
      const asciiBlast = [`\n  *  \n`, `\\ | /\n- O -\n/ | \\`, `* # *\n# @ #\n* # *`, ` . . \n.   .\n . . `];
      let frame = 0; el.style.color = '#ffbd2e'; 
      const blastInterval = setInterval(() => {
        if (frame < asciiBlast.length) {
          if (frame === 1) el.style.color = '#ff5f56'; 
          if (frame === 3) el.style.color = 'var(--text-dimmer)'; 
          el.innerHTML = `<span style="font-size:14px; font-weight:bold;">${asciiBlast[frame]}</span>`;
          frame++;
        } else {
          clearInterval(blastInterval); isRemoved = true; el.remove();
          const idx = activePets.indexOf(petData); if (idx > -1) activePets.splice(idx, 1);
        }
      }, 120);
    }
  };
  ...
  function drift(){
    ...
    if (state === 'normal') {
      x += vx; y += vy;
      if(x <= 0) { x = 0; vx = Math.abs(vx); } else if(x >= window.innerWidth - 60) { x = window.innerWidth - 60; vx = -Math.abs(vx); }
      if(y <= 80) { y = 80; vy = Math.abs(vy); } else if(y >= document.body.scrollHeight - 60) { y = document.body.scrollHeight - 60; vy = -Math.abs(vy); }
    } else if (state === 'chase') {
      const dx = mouseX - x - 20, dy = mouseY - y - 20, dist = Math.hypot(dx, dy);
      if(dist > 0) { x += (dx/dist)*4.5; y += (dy/dist)*4.5; }
      for (let other of activePets) {
         if (other !== petData && Math.hypot(x - other.x, y - other.y) < 40) {
             if (other.state === 'chase') { petData.explode(); other.explode(); break; } 
             else if (other.state === 'blackhole') { petData.explode(); break; }
         }
      }
    } 
    el.style.left = x + 'px'; el.style.top = y + 'px';
    if(state !== 'blasted' && state !== 'blackhole') requestAnimationFrame(drift);
  }
  requestAnimationFrame(drift);

  el.addEventListener('pointerdown', (e) => {
    e.stopPropagation(); e.preventDefault();
    if (isArmed || state === 'chase') { petData.explode(); return; }
    
    if (species.tip === "i am unstable" && state === 'normal') {
      state = 'meltdown'; el.style.color = '#ffbd2e';
      const wildFrames = [[" (\\___/) ", " ( ò_ó ) ", " />   <\\"], ["  |\\____/|  ", " / @    @ \\ ", " |  \\__/  | ", "  \\______/  "], ["   |\\______/|   ", "  / #      # \\  ", "  |  VVVVVV  |  ", "  |  ^^^^^^  |  ", "   \\________/   "]];
      let frameIdx = 0, ticks = 0;
      const jitter = setInterval(() => {
        el.style.transform = `translate(${(Math.random()-0.5)*14}px, ${(Math.random()-0.5)*14}px)`;
        if (ticks % 8 === 0 && frameIdx < wildFrames.length) {
          el.innerHTML = `<span class="tip" style="opacity:1; color:#ffbd2e">MELTDOWN!</span>${wildFrames[frameIdx].join('\n')}`;
      ...
      setTimeout(() => { clearInterval(jitter); el.style.transform = 'none'; petData.explode(); }, 1500);
      return;
    }

    if(species.tip === "do not touch" && state === 'normal') {
      state = 'blackhole'; el.style.color = '#c4b5fd';
      el.innerHTML = `<div class="tornado-vortex">  \\~~~/\n  } @ {\n  /~~~\\</div><span class="tip" style="opacity:1; color:#c4b5fd">THE VOID</span>{0,0}\n/)_)\\\n-"-"-`;
      const targets = document.querySelectorAll('.prompt-line, .history-line, .history-output, #hero-name, .hero-role, .lede p, .links a, .section-label, .job-head, .job-desc, .job-link, .divider, .row, .about-text p, #portrait, .contact-row, footer, .job-image-wrapper');
      targets.forEach(target => {
         const rect = target.getBoundingClientRect(), dx = (x + el.offsetWidth/2) - (rect.left + rect.width/2), dy = (y + el.offsetHeight/2) - (rect.top + rect.height/2);
         const itemObj = { el: target, originalTransition: target.style.transition || '', scrambler: null };
         eatenItems.push(itemObj);
         setTimeout(() => {
           itemObj.scrambler = scrambleTextNode(target);
           target.style.animation = 'none'; // Unlocks the terminal reveal freeze
           target.style.transition = 'transform 1.5s cubic-bezier(0.55, 0.085, 0.68, 0.53), opacity 1.5s, filter 1.5s';
           target.style.transform = `translate(${dx}px, ${dy}px) scale(0) rotate(1080deg)`;
           target.style.opacity = '0'; target.style.filter = 'blur(8px)';
         }, Math.random() * 1500);
      });
      setTimeout(() => {
         isSystemOffline = true; rebootBuffer = "";
         document.getElementById('reboot-input').textContent = "";
         document.getElementById('reboot-sys-msg').textContent = "SYSTEM OFFLINE. AWAITING COMMAND.";
         document.getElementById('reboot-terminal').style.display = 'block';
         const mobileInput = document.getElementById('reboot-mobile-input');
         if(mobileInput) { mobileInput.value = ""; mobileInput.focus({ preventScroll: true }); }
      }, 3000);
      triggerRestore = () => {
         if (!isRemoved) { el.innerHTML = `<span class="tip" style="opacity:1; color:#27c93f">SYSTEM REBOOTING...</span>{0_0}\n/)_)\\\n-"-"-`; el.style.color = '#27c93f'; }
         eatenItems.forEach(item => {
            setTimeout(() => {
              item.el.style.transition = 'transform 1s cubic-bezier(0.175, 0.885, 0.32, 1.275), opacity 1s, filter 1s';
              item.el.style.transform = `translate(0px, 0px) scale(1) rotate(0deg)`; item.el.style.opacity = '1'; item.el.style.filter = 'blur(0px)';
              setTimeout(() => { if(item.scrambler) item.scrambler.stop(); item.el.style.transition = item.originalTransition; }, 1000); 
            }, Math.random() * 1000);
         });
         setTimeout(() => {
            if (!isRemoved) { state = 'normal'; el.style.color = 'var(--text-dimmer)'; el.innerHTML = `<span class="tip">${species.tip}</span>${species.art.join('\n')}`; requestAnimationFrame(drift); }
            eatenItems = [];
         }, 2500);
      };
      return; 
    ...
    if(state !== 'normal') return;
    state = ['blast', 'chase', 'chase', 'bloat'][Math.floor(Math.random() * 4)];
    if (state === 'blast') petData.explode();
    else if (state === 'bloat') {
      el.style.transition = 'transform 2s ease-in, color 1s'; el.style.color = '#ffbd2e'; el.style.transform = 'scale(10)';
      el.innerHTML = `<span class="tip" style="opacity:1;">too full!</span>${species.art.join('\n')}`;
      setTimeout(() => { el.innerHTML = '<span style="color:#ff5f56; font-size:12px;">💥</span>'; setTimeout(()=> petData.remove(), 300); }, 2000);
    } else if (state === 'chase') {
      el.style.color = '#ff5f56';
      el.innerHTML = `<span class="tip" style="opacity:1; color:#ff5f56">DESTROY!</span>${species.art.join('\n').replace(/o\.o/g, 'ò.ó').replace(/o  o/g, 'ò  ó').replace(/0,0/g, '>,<')}`;
      spawnGun();
    } 
  });
}
window.addEventListener('load', () => {
  PET_SPECIES.forEach(species => spawnPet(window.innerWidth*(0.1+Math.random()*0.8), 100+Math.random()*(window.innerHeight-200), species));
  for(let i=0; i<2; i++) spawnPet(window.innerWidth*(0.1+Math.random()*0.8), 100+Math.random()*(window.innerHeight-200), PET_SPECIES[Math.floor(Math.random()*PET_SPECIES.length)]);
});

/* ---------------- REBOOT & MATRIX ---------------- */
const rebootMobileInput = document.createElement('input');
rebootMobileInput.id = 'reboot-mobile-input';
rebootMobileInput.style.cssText = 'position:absolute; opacity:0; width:1px; height:1px; border:none;';
rebootMobileInput.setAttribute('autocomplete', 'off'); rebootMobileInput.setAttribute('spellcheck', 'false');
document.body.appendChild(rebootMobileInput);
...
rebootMobileInput.addEventListener('keydown', (e) => {
  if (!isSystemOffline) return;
  if (e.key === 'Enter') {
    if (['reboot', 'sudo restore'].includes(rebootBuffer.trim().toLowerCase())) {
       isSystemOffline = false; document.getElementById('reboot-terminal').style.display = 'none';
       rebootMobileInput.blur(); startMatrixReboot();
    } else {
       document.getElementById('reboot-sys-msg').textContent = "COMMAND NOT RECOGNIZED. TRY 'reboot'";
       document.getElementById('reboot-sys-msg').style.color = "#ff5f56";
       rebootBuffer = ""; rebootMobileInput.value = ""; document.getElementById('reboot-input').textContent = "";
    }
  }
});
document.addEventListener('click', () => { if (isSystemOffline) rebootMobileInput.focus({ preventScroll: true }); });

document.addEventListener('keydown', (e) => {
  if (!isSystemOffline || document.activeElement === rebootMobileInput) return;
  const inputEl = document.getElementById('reboot-input'), sysMsg = document.getElementById('reboot-sys-msg');
  if (e.key === 'Backspace') {
    rebootBuffer = rebootBuffer.slice(0, -1); inputEl.textContent = rebootBuffer;
    sysMsg.textContent = "SYSTEM OFFLINE. AWAITING COMMAND."; sysMsg.style.color = "var(--text-dimmer)";
  } else if (e.key === 'Enter') {
    if (['reboot', 'sudo restore'].includes(rebootBuffer.trim().toLowerCase())) {
       isSystemOffline = false; document.getElementById('reboot-terminal').style.display = 'none'; startMatrixReboot();
    } else {
       sysMsg.textContent = "COMMAND NOT RECOGNIZED. TRY 'reboot'"; sysMsg.style.color = "#ff5f56";
       rebootBuffer = ""; inputEl.textContent = "";
    }
  } else if (e.key.length === 1 && rebootBuffer.length < 25) { 
    rebootBuffer += e.key; inputEl.textContent = rebootBuffer;
    sysMsg.textContent = "SYSTEM OFFLINE. AWAITING COMMAND."; sysMsg.style.color = "var(--text-dimmer)";
  }
});
...
function startMatrixReboot() {
  ...
  canvas.width = window.innerWidth; canvas.height = window.innerHeight; canvas.style.display = 'block'; canvas.style.opacity = '1';
  for (let i = activePets.length - 1; i >= 0; i--) if (!['blackhole','choking'].includes(activePets[i].state)) { activePets[i].remove(); activePets.splice(i, 1); }
  document.querySelectorAll('.ascii-gun').forEach(g => g.remove());
  isArmed = false; document.body.classList.remove('armed'); const uiCounter = document.getElementById('ammo-counter'); if(uiCounter) uiCounter.remove();
  for(let i=0; i<6-activePets.length; i++) spawnPet(window.innerWidth*(0.1+Math.random()*0.8), 100+Math.random()*(window.innerHeight-200), PET_SPECIES[Math.floor(Math.random()*PET_SPECIES.length)]);
  
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789@#$%^&*'.split(''), fontSize = 14, drops = Array(Math.floor(canvas.width / fontSize)).fill(1);
  const rainInterval = setInterval(() => {
    ctx.fillStyle = 'rgba(10, 10, 12, 0.1)'; ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.fillStyle = '#27c93f'; ctx.font = fontSize + 'px "JetBrains Mono", monospace';
    for(let i=0; i<drops.length; i++) {
      ctx.fillText(chars[Math.floor(Math.random() * chars.length)], i * fontSize, drops[i] * fontSize);
      if(drops[i] * fontSize > canvas.height && Math.random() > 0.975) drops[i] = 0; drops[i]++;
    }
  }, 33);
  setTimeout(() => { if(triggerRestore) triggerRestore(); }, 500);
  setTimeout(() => { canvas.style.opacity = '0'; setTimeout(() => { clearInterval(rainInterval); canvas.style.display = 'none'; }, 1500); }, 2500);
}

/* ---------------- FETCH API CRESTS ---------------- */
(function() {
  function initAsciiToggle(cellId, textFilePath, logoClassName) {
    const cell = document.getElementById(cellId); if (!cell) return;
    const titleDiv = cell.querySelector('.what'); if (!titleDiv) return;
    const logoEl = document.createElement('pre'); logoEl.className = logoClassName; logoEl.style.display = 'none'; logoEl.textContent = '[ DOWNLOADING PROTOCOL... ]';
    cell.insertBefore(logoEl, titleDiv);
    let showingLogo = false, isLoaded = false;
    cell.addEventListener('click', async () => {
      showingLogo = !showingLogo;
      if (showingLogo) {
        titleDiv.style.display = 'none'; logoEl.style.display = 'block';
        if (!isLoaded) {
          try {
            await new Promise(r => setTimeout(r, 300)); 
            const response = await fetch(textFilePath); if (!response.ok) throw new Error('File missing');
            logoEl.textContent = await response.text(); isLoaded = true;
          } catch (error) { logoEl.textContent = '[ ERROR: DATA CORRUPTED OR MISSING ]'; }
        }
      } else { titleDiv.style.display = 'block'; logoEl.style.display = 'none'; }
    });
  }
  initAsciiToggle('bmsceCell', 'bmsce.txt', 'bmsce-logo'); 
  initAsciiToggle('rvceCell', 'rvpu.txt', 'rvce-logo');    
...
(function initPortrait() {
  const imagePaths = ['1.png', '2.png', '3.png', '4.png', '5.png', '6.png', '7.png'];
  const canvas = document.getElementById('portraitCanvas'); if (!canvas) return;
  const ctx = canvas.getContext('2d', { willReadFrequently: true });
  let allFrameCoordinates = [], activeFrameIndex = 3; 

  function loadImage(url) { return new Promise((res, rej) => { const img = new Image(); img.onload = () => res(img); img.onerror = () => rej(`Failed: ${url}`); img.src = url; }); }
  function extractPoints(image) {
    ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.drawImage(image, 0, 0, canvas.width, canvas.height);
    const imgData = ctx.getImageData(0, 0, canvas.width, canvas.height).data, points = [];
    for (let y=0; y<canvas.height; y+=4) {
      for (let x=0; x<canvas.width; x+=4) {
        const i = (y * canvas.width + x) * 4;
        const bright = (imgData[i] + imgData[i+1] + imgData[i+2])/3;
        if (imgData[i+3] > 40 && bright > 80) { const r = (bright/255)*2.2; if (r > 0.4) points.push({ x, y, size: r }); }
      }
    }
    return points;
  }
  async function setup() { try { const imgs = await Promise.all(imagePaths.map(loadImage)); allFrameCoordinates = imgs.map(extractPoints); renderFrame(); } catch (err) {} }
  function updateFrameIndex(cx) { activeFrameIndex = Math.min(imagePaths.length - 1, Math.floor(Math.max(0, Math.min(1, (cx / window.innerWidth - 0.1) * 1.25)) * imagePaths.length)); renderFrame(); }
  window.addEventListener('mousemove', e => updateFrameIndex(e.clientX)); window.addEventListener('touchmove', e => { if (e.touches.length > 0) updateFrameIndex(e.touches[0].clientX); });
  function renderFrame() {
    ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#7dd3fc';
    const tgts = allFrameCoordinates[activeFrameIndex]; if (!tgts) return;
    for (let i=0; i<tgts.length; i++) { ctx.beginPath(); ctx.arc(tgts[i].x, tgts[i].y, tgts[i].size, 0, Math.PI * 2); ctx.fill(); }
  }
  document.getElementById('hero-name').addEventListener('click', () => setTimeout(renderFrame, 50));
  setup();
})();
...
(function initBootSequence() {
  const bootScreen = document.getElementById('rtos-boot');
  if (!bootScreen || sessionStorage.getItem('rtos_booted')) { if (bootScreen) bootScreen.remove(); return; }
  document.body.classList.add('booting');
  const logsContainer = document.getElementById('rtos-logs'), mainScreen = document.getElementById('rtos-main'), loadingWrapper = document.getElementById('rtos-loading-wrapper');
  const genHex = () => '0x' + Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0').toUpperCase();
  const fastLogs = Array.from({length: 15}, () => `${genHex()} ALLOCATING MEMORY SECTORS... OK`);
  const slowLogs = ["MOUNTING KERNEL... [SUCCESS]", "ESTABLISHING NEURAL LINK... [SUCCESS]", "DECRYPTING PAYLOAD...", "LOADING RTOS CORE..."];
  let logIndex = 0, isFastPhase = true;

  function renderLog(text, color = "var(--text-dimmer)") {
    const p = document.createElement('div'); p.style.color = color; p.textContent = `[${genHex()}] > ${text}`; logsContainer.appendChild(p);
    if (logsContainer.children.length > 20) logsContainer.removeChild(logsContainer.firstChild);
  }
  function bootLoop() {
    if (isFastPhase) {
      if (logIndex < fastLogs.length) { renderLog(fastLogs[logIndex]); logIndex++; setTimeout(bootLoop, 30); } 
      else { isFastPhase = false; logIndex = 0; renderLog("--------------------------------", "var(--accent)"); setTimeout(bootLoop, 300); }
    } else {
      if (logIndex < slowLogs.length) { renderLog(slowLogs[logIndex], "var(--text-dim)"); logIndex++; setTimeout(bootLoop, 200 + Math.random() * 400); } 
      else renderProgressBar();
    }
  }
  function renderProgressBar() {
    let progress = 0; const pBar = document.createElement('div'); pBar.style.cssText = "color:#27c93f; margin-top:15px;"; logsContainer.appendChild(pBar);
    const interval = setInterval(() => {
      progress = Math.min(100, progress + Math.floor(Math.random() * 12) + 4);
      const filled = Math.floor(progress / 5); pBar.textContent = `SYSTEM BOOT: [${'█'.repeat(filled) + '░'.repeat(20 - filled)}] ${progress}%`;
      if (progress === 100) { clearInterval(interval); setTimeout(() => { loadingWrapper.style.display = 'none'; mainScreen.style.display = 'flex'; document.body.classList.add('show-aura'); }, 500); }
    }, 70);
  }
  setTimeout(bootLoop, 400);
  function exitBoot() { bootScreen.style.opacity = '0'; document.body.classList.remove('booting', 'show-aura'); sessionStorage.setItem('rtos_booted', 'true'); setTimeout(() => bootScreen.remove(), 800); }
  document.addEventListener('keydown', e => { if (e.key === 'Enter' && mainScreen.style.display === 'flex') exitBoot(); });
  bootScreen.addEventListener('click', () => { if (mainScreen.style.display === 'flex') exitBoot(); });
})();

/* ---------------- CUSTOM CURSOR TRACKING ---------------- */
(function initCustomCursor() {
  const cursor = document.getElementById('custom-cursor'), cursorTrail = document.getElementById('custom-cursor-trail');
  if (!cursor || !cursorTrail) return;
  const auraChars = ['@', '#', '$', '%', '*', '+', '=', '!', '?', '&', '1', '0'];
  window.addEventListener('mousemove', e => {
    cursor.style.left = `${e.clientX}px`; cursor.style.top = `${e.clientY}px`;
    cursorTrail.style.left = `${e.clientX}px`; cursorTrail.style.top = `${e.clientY}px`;
    if (document.body.classList.contains('show-aura') && Math.random() > 0.5) { 
        const p = document.createElement('div'); p.className = 'cursor-aura-particle'; p.textContent = auraChars[Math.floor(Math.random() * auraChars.length)];
        p.style.left = `${e.clientX}px`; p.style.top = `${e.clientY}px`;
        p.style.setProperty('--dx', `${(Math.random() - 0.5) * 80}px`); p.style.setProperty('--dy', `${(Math.random() - 0.5) * 80}px`);
        document.body.appendChild(p); setTimeout(() => p.remove(), 1000);
    }
  });
  document.querySelectorAll('a, mark, #hero-name, .job-cell, .edu-cell, .reload, .rtos-char, .boot-prompt, .pet, .ascii-gun, .job-image-wrapper').forEach(el => {
    el.addEventListener('mouseenter', () => document.body.classList.add('hovering')); el.addEventListener('mouseleave', () => document.body.classList.remove('hovering'));
  });
  const hn = document.getElementById('hero-name');
  if (hn) { hn.addEventListener('mouseenter', () => document.body.classList.add('show-aura')); hn.addEventListener('mouseleave', () => document.body.classList.remove('show-aura')); }
})();

/* ---------------- INTERACTIVE TERMINAL ---------------- */
(function initTerminal() {
  const terminalContainer = document.getElementById('interactive-terminal'), input = document.getElementById('cmd-input'), display = document.getElementById('cmd-text'), activeLine = document.getElementById('active-prompt-line'), wrap = document.querySelector('.wrap'); 
  let historyContainer = document.getElementById('terminal-history');
  if (!input || !display || !terminalContainer || !wrap) return;
  
  let isRevealed = sessionStorage.getItem('portfolio_revealed') === 'true';
  if (!isRevealed) document.body.classList.add('terminal-locked');
  else {
    const bottomTerminal = document.createElement('div'); bottomTerminal.id = 'interactive-terminal-bottom'; bottomTerminal.style.cssText = "margin-top:40px; margin-bottom:26px; font-family:'JetBrains Mono',monospace; font-size:13px;";
    const newHistory = document.createElement('div'); newHistory.id = 'terminal-history-bottom'; newHistory.style.cssText = "color:var(--text-dim); white-space:pre-wrap; line-height:1.5;";
    bottomTerminal.appendChild(newHistory); bottomTerminal.appendChild(activeLine); wrap.appendChild(bottomTerminal); historyContainer = newHistory; 
  }
  
  let commandHistory = [], historyPos = 0;
  const validCommands = ['help', 'ls', 'whoami', 'clear', './portfolio', './hobbies'];
  
  document.addEventListener('click', (e) => {
    if (window.getSelection().toString().length > 0) return;
    if (e.target.closest('a, button, .pet, .edu-cell, .reload, .ascii-gun, #hero-name, .job-image-wrapper')) return;
    input.focus({ preventScroll: true });
  });
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === 'Tab') {
      e.preventDefault(); 
      const matches = validCommands.filter(cmd => cmd.startsWith(input.value.toLowerCase()));
      if (matches.length === 1) { input.value = matches[0]; display.textContent = input.value; } 
      else if (matches.length > 1) {
        historyContainer.insertAdjacentHTML('beforeend', `<div class="history-line"><span class="user">ravi@teja</span> ~ % <span style="color: var(--text)">${input.value.toLowerCase()}</span></div>`);
        historyContainer.insertAdjacentHTML('beforeend', `<div class="history-output">${matches.join('    ')}</div>`);
        setTimeout(() => window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' }), 50);
      }
      return;
    }
    if (e.key === 'Enter') {
      const cmd = input.value.trim();
      if (cmd) { commandHistory.push(cmd); historyPos = commandHistory.length; processCommand(cmd); } 
      else { historyContainer.insertAdjacentHTML('beforeend', `<div class="history-line"><span class="user">ravi@teja</span> ~ % </div>`); setTimeout(() => window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' }), 50); }
      input.value = ''; display.textContent = '';
    } 
    else if (e.key === 'ArrowUp') { e.preventDefault(); if (historyPos > 0) { historyPos--; input.value = commandHistory[historyPos]; display.textContent = input.value; } } 
    else if (e.key === 'ArrowDown') { e.preventDefault(); if (historyPos < commandHistory.length - 1) { historyPos++; input.value = commandHistory[historyPos]; display.textContent = input.value; } else { historyPos = commandHistory.length; input.value = ''; display.textContent = ''; } }
  });
  
  function processCommand(cmd) {
    historyContainer.insertAdjacentHTML('beforeend', `<div class="history-line"><span class="user">ravi@teja</span> ~ % <span style="color: var(--text)">${cmd}</span></div>`);
    let output = ''; const lowerCmd = cmd.toLowerCase();
    const helpText = "AVAILABLE COMMANDS:\n  help        - Show this menu\n  ls          - List directory contents\n  ./portfolio - Execute portfolio protocol\n  ./hobbies   - Display personal interests\n  whoami      - Print current user\n  clear       - Clear terminal output";

    switch (lowerCmd) {
      case 'help': output = helpText; break;
      case 'ls': output = "drwxr-xr-x  projects/\ndrwxr-xr-x  experience/\ndrwxr-xr-x  education/\n-rwxr-xr-x  portfolio.sh\n-rw-r--r--  hobbies.txt\n-rw-r--r--  contact.asc"; break;
      case './portfolio': output = isRevealed ? "[ PORTFOLIO PROTOCOL ALREADY ACTIVE ]" : "[INITIALIZING PORTFOLIO PROTOCOLS...]\nLoading components...\nBypassing security protocols...\nAccess granted."; break;
      case './hobbies': output = "HOBBIES.TXT:\n> Tinkering with distributed systems\n> Reverse-engineering encryption algorithms\n> [Add your other hobbies here!]\n> [And here!]"; break;
      case 'whoami': output = "Ravi Teja S\nFinal-year ISE student @ BMSCE\nSystems & Infrastructure Engineer"; break;
      case 'clear':
        const topHistory = document.getElementById('terminal-history'); if (topHistory) topHistory.innerHTML = '';
        if (isRevealed) {
            isRevealed = false; sessionStorage.setItem('portfolio_revealed', 'false'); document.body.classList.add('terminal-locked');
            const topTerminal = document.getElementById('interactive-terminal'); if (topTerminal) topTerminal.appendChild(activeLine);
            const bottomTerminal = document.getElementById('interactive-terminal-bottom'); if (bottomTerminal) bottomTerminal.remove();
            historyContainer = topHistory;
            [document.getElementById('hero-name'), document.querySelector('.hero-role'), document.querySelector('.lede'), document.querySelector('.links'), ...document.querySelectorAll('section:not(#intro)'), document.querySelector('footer')].forEach(el => { if(el) { el.style.opacity = ''; el.style.animation = ''; } });
        } else historyContainer.innerHTML = '';
        break;
      default:
        const match = validCommands.find(c => c.includes(lowerCmd) || lowerCmd.includes(c));
        output = (match && lowerCmd.length > 1) ? `zsh: correct '${cmd}' to '${match}' [nyae]? \nType 'help' to see available commands.` : `zsh: command not found: ${cmd}\n\n${helpText}`;
    }
    
    if (output && lowerCmd !== 'clear') { historyContainer.insertAdjacentHTML('beforeend', `<div class="history-output">${output}</div>`); }
    
    if (!isRevealed && lowerCmd === './portfolio') {
      isRevealed = true; sessionStorage.setItem('portfolio_revealed', 'true'); document.body.classList.remove('terminal-locked');
      const bottomTerminal = document.createElement('div'); bottomTerminal.id = 'interactive-terminal-bottom'; bottomTerminal.style.cssText = "margin-top:40px; margin-bottom:26px; font-family:'JetBrains Mono',monospace; font-size:13px;";
      const newHistory = document.createElement('div'); newHistory.id = 'terminal-history-bottom'; newHistory.style.cssText = "color:var(--text-dim); white-space:pre-wrap; line-height:1.5;";
      bottomTerminal.appendChild(newHistory); bottomTerminal.appendChild(activeLine); wrap.appendChild(bottomTerminal); historyContainer = newHistory;
      
      [document.getElementById('hero-name'), document.querySelector('.hero-role'), document.querySelector('.lede'), document.querySelector('.links'), ...document.querySelectorAll('section:not(#intro)'), document.querySelector('footer')].forEach((el, index) => {
        if(el) { el.style.opacity = '0'; el.style.animation = `terminalReveal 0.6s cubic-bezier(0.25, 0.8, 0.25, 1) forwards ${index * 0.1}s`; }
      });
    }
    setTimeout(() => { if (lowerCmd === 'clear') window.scrollTo(0, 0); else window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' }); }, 50);
  }
})();